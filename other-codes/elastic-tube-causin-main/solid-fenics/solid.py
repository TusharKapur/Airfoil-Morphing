...
def clamped_boundary(x, on_boundary):
    return on_boundary and ((abs(x[0]) < tol) or (abs(x[0] - pipe_length) < tol))
...
def neumann_boundary(x, on_boundary):
    ...
    """
    return on_boundary and (abs(x[1] - pipe_inner_radius) < tol)
...
pipe_thickness = 0.1
pipe_length = 3
rho = 3000  # Default 3000
E = 40000 # Default 4000000
nu = 0.3
...
n_x_Direction = 30
n_y_Direction = 4
mesh = RectangleMesh(Point(0, pipe_inner_radius), Point(pipe_length, (pipe_inner_radius+pipe_thickness)), n_x_Direction, n_y_Direction)

h = Constant(pipe_length / n_x_Direction)
