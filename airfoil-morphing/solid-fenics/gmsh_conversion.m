size_airfoil = 1.5E-01;    % Size of mesh near main airfoil
...
curve_loop_no = 1;
fprintf(fid, 'Physical Curve("airfoil", %i) = {', curve_loop_no);
line_no_temp = line_airfoil;
for n = 2:size(a_points(:,1))
    fprintf(fid, "%i, ", line_no_temp);
    line_no_temp = line_no_temp + 1;
...
fprintf(fid, 'Physical Surface("flap", 1) = {1}; \n');
...
% fprintf(fid, 'Physical Surface("flap", 1) = {%s}; \n', airfoil_surface_list);
...
% system(['gmsh mesh.geo -2 mesh.msh -format msh2']);
